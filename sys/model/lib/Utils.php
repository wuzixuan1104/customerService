...
class Utils
{
	...
	public static function add_where(&$wheres=array(), $where, $conjuction='AND')
	{
		if (is_array($where))
		{
			if (empty($wheres))
				$wheres = array_flatten($where);
			else
			{
				$wheres[0] .= " $conjuction " . array_shift($where);
				$wheres[] = array_flatten($where);
			}
		}
		elseif (is_string($where))
			$wheres[0] .= " $conjuction $where";
